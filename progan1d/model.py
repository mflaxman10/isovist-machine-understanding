...
class ConvBlock(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, padding, kernel_size2=None, padding2=None, pixel_norm=True, padding_mode='zeros', padding_mode2=None):
        super().__init__()
        ...
        pad_mode1 = padding_mode
        pad_mode2 = padding_mode
        if padding_mode2 is not None:
            pad_mode2 = padding_mode2

        convs = [EqualConv1d(in_channel, out_channel, kernel1, padding=pad1, padding_mode=pad_mode1)]
        if pixel_norm:
            convs.append(PixelNorm())
        convs.append(nn.LeakyReLU(0.1))
        convs.append(EqualConv1d(out_channel, out_channel, kernel2, padding=pad2, padding_mode=pad_mode2))
        if pixel_norm:
            convs.append(PixelNorm())
        convs.append(nn.LeakyReLU(0.1))
...
class Generator(nn.Module):
    def __init__(self, input_code_dim=16, in_channel=128, pixel_norm=True, tanh=True, kernel_size=3, pad_mode='zeros'):
        super().__init__()
        ...
        self.progression_4 = ConvBlock(in_channel, in_channel, kernel_size, pad, pixel_norm=pixel_norm, padding_mode=pad_mode)
        self.progression_8 = ConvBlock(in_channel, in_channel, kernel_size, pad, pixel_norm=pixel_norm, padding_mode=pad_mode)
        self.progression_16 = ConvBlock(in_channel, in_channel, kernel_size, pad, pixel_norm=pixel_norm, padding_mode=pad_mode)
        self.progression_32 = ConvBlock(in_channel, in_channel, kernel_size, pad, pixel_norm=pixel_norm, padding_mode=pad_mode)
        self.progression_64 = ConvBlock(in_channel, in_channel//2, kernel_size, pad, pixel_norm=pixel_norm, padding_mode=pad_mode)
        self.progression_128 = ConvBlock(in_channel//2, in_channel//4, kernel_size, pad, pixel_norm=pixel_norm, padding_mode=pad_mode)
        self.progression_256 = ConvBlock(in_channel//4, in_channel//4, kernel_size, pad, pixel_norm=pixel_norm, padding_mode=pad_mode)
...
class Discriminator(nn.Module):
    def __init__(self, feat_dim=128, kernel_size=3, pad_mode='zeros'):
        super().__init__()

        pad = int((kernel_size-1)/2)

        self.progression = nn.ModuleList([ConvBlock(feat_dim//4, feat_dim//4, kernel_size, pad, padding_mode=pad_mode),
                                          ConvBlock(feat_dim//4, feat_dim//2, kernel_size, pad, padding_mode=pad_mode),
                                          ConvBlock(feat_dim//2, feat_dim, kernel_size, pad, padding_mode=pad_mode),
                                          ConvBlock(feat_dim, feat_dim, kernel_size, pad, padding_mode=pad_mode),
                                          ConvBlock(feat_dim, feat_dim, kernel_size, pad, padding_mode=pad_mode),
                                          ConvBlock(feat_dim, feat_dim, kernel_size, pad, padding_mode=pad_mode),
                                          ConvBlock(feat_dim+1, feat_dim, kernel_size, pad, 4, 0, padding_mode=pad_mode)])
...
class Encoder(nn.Module):
    def __init__(self, feat_dim=128, latent_dim=16, kernel_size=3, pad_mode='zeros'):
        super().__init__()
        ...
        # self.linear = EqualLinear(feat_dim, 1)
        self.new_linear = nn.Linear(feat_dim, latent_dim)
    ...
    def forward(self, input, step=0, alpha=-1):
        ...
        # print(input.size(), out.size(), step)
        # out = self.linear(out)
        out = self.new_linear(out)


        return out

class EncoderFC(nn.Module):
    def __init__(self, feat_dim=128, latent_dim=16):
        super().__init__()
        self.latent_dim = latent_dim
        ...
        # self.linear = EqualLinear(feat_dim, 1)
        self.new_linear = nn.Sequential(
            nn.Linear(feat_dim, feat_dim//2), # 256 --> 128
            nn.Linear(feat_dim//2, feat_dim//4), # 128 --> 64
            nn.Linear(feat_dim//4, feat_dim//8), # 64 --> 32
            nn.Linear(feat_dim//8,  latent_dim) # 128 --> 64
            )
