...
class Discriminator(nn.Module):
    ...
    def __init__(self, latent_dim=16, starting_nets=16):
        super(Discriminator, self).__init__()
        self.model = nn.Sequential(

            nn.Linear(latent_dim, starting_nets*2**2, bias=True),
            nn.ReLU(inplace=True),

            nn.Linear(starting_nets*2**2, starting_nets*2**2, bias=True),
            nn.ReLU(inplace=True),

            nn.Linear(starting_nets*2**2,starting_nets*2, bias=True),
            nn.ReLU(inplace=True),

            nn.Linear(starting_nets*2, starting_nets, bias=True),
            nn.ReLU(inplace=True),

            nn.Linear(starting_nets, 1, bias=True),
            nn.Sigmoid()
        )
